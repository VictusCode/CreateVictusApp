...
namespace Api.UnitTests.Infrastructure.Filters
{
  public class ApiKeyAuthorizationFilterTests
  {
    ...
    public ApiKeyAuthorizationFilterTests()
    {
      ...
      _featureManagerMock.Setup(x => x.IsEnabledAsync(FeatureFlags.ApiKey.ToString())).ReturnsAsync(true);
    ...
    {
      //given
      _featureManagerMock.Setup(x => x.IsEnabledAsync(FeatureFlags.ApiKey.ToString())).ReturnsAsync(false);
    ...
    private AuthorizationFilterContext GetMockedContext(string secretKey)
    {
      ...
      httpRequestMock.Setup(mock => mock.Headers).Returns(headers).Callback(() => _headersAccessed = true);
