...
namespace Api.UnitTests.Controllers
{
  public class EmployeesControllerTests : ControllerTestsBase<EmployeesController>
  {
    ...
    public async Task GetAll_should_return_expected_results(List<EmployeeDto> employees)
    {
      ...
      result.Value.Should().BeAssignableTo<IEnumerable<EmployeeDto>>().And.BeEquivalentTo(employees);
    ...
    public async Task Get_should_return_Ok_with_expected_result_when_repository_return_object()
    {
      ...
      _employeeRepositoryMock
        .Setup(x => x.GetByIdAsync(It.IsAny<int>(), It.IsAny<CancellationToken>()))
        .ReturnsAsync(new EmployeeDto { Id = empId, LastName = lastName, });
      ...
      emp.LastName.Should().NotBeNullOrEmpty().And.Be(lastName);
    }
    ...
    public async Task Delete_should_call_DeleteByIdAsync_onto_repository()
    {
      ...
      _employeeRepositoryMock.Verify(
        x => x.DeleteByIdAsync(empId, It.IsAny<CancellationToken>()),
        Times.Once
      );
    }
    ...
    public async Task Delete_should_return_NotFound_when_repository_return_false()
    {
      //given
      _employeeRepositoryMock
        .Setup(x => x.DeleteByIdAsync(It.IsAny<int>(), It.IsAny<CancellationToken>()))
        .ReturnsAsync(false);
    ...
    [Theory, AutoData]
    public async Task Put_should_return_NotFound_when_repository_return_null(
      int empId,
    ...
    )
    {
      //given
      _employeeRepositoryMock
        .Setup(x => x.UpdateAsync(empId, employeePutDto, It.IsAny<CancellationToken>()))
        .ReturnsAsync(() => null)
        .Verifiable();
    ...
    [Theory, AutoData]
    public async Task Put_should_return_Ok_with_result_when_update_finished_with_success(
      int empId,
      EmployeePutDto employeePutDto,
    ...
    )
    {
      ...
      //then
      result.Value.Should().BeAssignableTo<EmployeeDto>().And.BeEquivalentTo(employee);

      _employeeRepositoryMock.Verify();
    }

    [Theory, AutoData]
    public async Task Post_should_return_Created_with_result_and_header_when_insert_finished_with_success(
    ...
    )
    {
      //given
      _employeeRepositoryMock
        .Setup(x => x.InsertAsync(employeePostDto, It.IsAny<CancellationToken>()))
        .ReturnsAsync(employee)
        .Verifiable();
    ...
    public async Task GetOldest_should_return_Santa_when_feature_enabled()
    {
      ...
      featureManagerMock
        .Setup(x => x.IsEnabledAsync(FeatureFlags.Santa.ToString()))
        .ReturnsAsync(true)
        .Verifiable();
      ...
      result.Should().NotBeNull();
      result.StatusCode.Should().Be(StatusCodes.Status200OK);
      result.Value.Should().BeAssignableTo<EmployeeDto>().Which.FirstName.Should().Be("Santa");
    }
  }
}
