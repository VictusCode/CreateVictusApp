...
namespace Db
{
  class Program
  {
    public static int Main(string[] args)
    {
      ...
      var scriptsAndCodePatternPattern = new Regex(
        upgradeOptions.ScriptsAndCodePattern ?? ".*",
        RegexOptions.IgnoreCase | RegexOptions.Compiled
      );

      var upgrader = DeployChanges
        .To.SqlDatabase(configuration.GetConnectionString("MsSqlDb"))
        .WithScriptsAndCodeEmbeddedInAssembly(
          Assembly.GetExecutingAssembly(),
          (fileName) => scriptsAndCodePatternPattern.IsMatch(fileName)
        )
