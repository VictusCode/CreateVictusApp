...
namespace Core.Repositories
{
  ...
  {
    public EmployeeRepository(EmployeesContext dbContext)
      : base(dbContext) { }
    ...
    public async Task<EmployeeDetailsDto> GetByIdWithDetailsAsync(
      object id,
    ...
    )
    {
      ...
      return new EmployeeDetailsDto
      {
        ...
        Gender = emp.Gender,
        Department = new DepartmentDto { Id = emp.Department.DeptNo, Name = emp.Department.DeptName, }
      };
    }

    public async Task<EmployeeDto> GetOldestAsync(CancellationToken cancellationToken)
    {
      var emp = await DbContext.Employees.OrderBy(x => x.BirthDate).FirstOrDefaultAsync(cancellationToken);
      if (emp == null)
      {
        return null;
      }
    ...
    public async Task<EmployeeDto> InsertAsync(
    ...
    {
      var emp = await DbContext.Employees.SingleOrDefaultAsync(x => x.EmpNo == id, cancellationToken);
      if (emp is null)
      {
        return null;
      }
