...
namespace BooksModule.Features.UpsertBook;
...
";
  ...
  public static async Task UpsertBookAsync(
  ...
  )
  {
    var id =
      ...
      ?? await db.ExecuteScalarAsync<int>(
        new CommandDefinition(_nextId, cancellationToken: cancellationToken)
      );

    await db.ExecuteAsync(
      new CommandDefinition(
        _upsertBook,
        new { id = id, title = book.Title, },
        cancellationToken: cancellationToken
      )
    );
  }
