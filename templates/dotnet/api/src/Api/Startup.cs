...
namespace Api
{
  public class Startup
  {
    ...
    {
      services.AddHttpContextAccessor().AddRouting(options => options.LowercaseUrls = true);
      ...
      services.AddDbContext<EmployeesContext>(
        options =>
          options.UseMySql(_configuration.GetConnectionString("MySqlDb"), ServerVersion.Parse("8.0")),
        contextLifetime: ServiceLifetime.Transient,
        optionsLifetime: ServiceLifetime.Singleton
      );
      services.AddDbContextPool<CarsContext>(
        options => options.UseSqlServer(_configuration.GetConnectionString("MsSqlDb")),
        poolSize: 10
      );
      ...
      var healthChecksBuilder = services.AddHealthChecks().AddBooksModule(_configuration);
    ...
    public virtual void Configure(IApplicationBuilder app, IWebHostEnvironment env)
    {
      ...
      {
        endpoints
          .MapHealthChecks("/healthz/live", new HealthCheckOptions { Predicate = _ => false, })
          .ShortCircuit();
        endpoints
          .MapHealthChecks(
            "/healthz/ready",
        ...
        endpoints
          .MapGet("/version", (VersionProvider provider) => provider.VersionEntries)
